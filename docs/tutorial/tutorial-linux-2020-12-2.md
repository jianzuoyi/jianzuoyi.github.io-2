...
有时候，我们运行一个软件或流程，有许多文件需要输出到一个目录中，但时间有点久，我们想实时查看文件的写入情况：

```bash
watch -n 3 -dc ls -l	# 追踪当前目录内容的变化，每3秒刷新一次
```
...
`find`是实时查找，如果需要更快的查询，可用`locate`：
...
```bash
ln -s file fileLink	# 软链接；删除源，另一个无法使用；（fileLink为新建的链接文件）
ln -sf file fileLink# 与上述命令的区别是：当fileLink已经存在时，强制创建
ln file fileHard	# 硬链接；删除一个，另一个仍可用
```
