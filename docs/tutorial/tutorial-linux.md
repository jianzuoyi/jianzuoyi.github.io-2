...
>
> 欢迎转载，转载请保留作者信息，谢谢。如果你对生物信息学感兴趣，欢迎关注我。
>
...
花了两天时间系统整理了Linux的常用命令，着实不容易。这些命令都是平时生物信息数据分析过程中经常用到的，所有命令的用法都经本人亲自测试。掌握这些命令，是每一个生信人基本的自我修养（不掌握这些命令，你就还没入门）。
...
```bash
cat run.sh | parallel -j 4	# 同时执行4个任务，生信中常通过这种方式并行执行多个任务
```
